...
class Transformer {
  ...
  #parser;
  /** @readonly */
  #dependenciesCache = new LoadingCache(
    /**
     * @param {string} id
     * @returns {Map<string, import("./Dependency")>}
     */
    (id) => {
      if (!this.#modules.has(id)) {
        return new Map();
      }
      return this.#parser.getDependenciesMap(id);
    }
  );

  /**
   * @param {ReadonlySet<string>} modules
   * @param {import("./Parser")} parser
   * @param {import("@babel/core").types} types
   */
  constructor(modules, parser, types) {
    this.#types = types;
    this.#parser = parser;
  ...
  transform(declaration) {
    ...
    if (importKind === "type" || !specifiers.length) {
      return;
    }

    const dependencies = this.#dependenciesCache.load(id);
