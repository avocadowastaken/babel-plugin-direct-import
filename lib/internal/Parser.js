...
const LoadingCache = require("./LoadingCache");
...
class Parser {
  ...
  /** @readonly */
  #resolverCache = new LoadingCache(
    /**
     * @param {[id: string, basedir: string | undefined]} input
     * @returns {string}
     */
    ([id, basedir]) => resolveModule(id, basedir),
   ...
   * @returns {string}
   */
  #resolveModule(id, basedir) {
    return this.#resolverCache.load([id, basedir]);
  }
   ...
   */
  #visitFile(filename) {
    return this.#fileCache.load(filename);
  }

  /**
   * @param {string} filename
   * @param {import("@babel/types").ExportAllDeclaration} node
   * @returns {Generator<Dependency, void, any>}
   */
  *#visitExportAllDeclaration(filename, node) {
    const dir = path.dirname(filename);
    const sourcePath = this.#resolveModule(node.source.value, dir);
